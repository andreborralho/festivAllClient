 ...
 */ (function ($) {
    ...
    var objId = function (obj) {
        if (!obj.jqScrollerId) obj.jqScrollerId = $.uuid();
        return obj.jqScrollerId;
    };
    ...
    var scroller = (function () {
        ...
        scrollerCore.prototype = {
            ...
            coreAddPullToRefresh: function (rEl) {
                ...
                if (this.refreshElement === null) {
                    ...
                    if (orginalEl !== null) {
                        ...
                        jqEl = jq("<div id='" + this.container.id + "_pulldown' class='jqscroll_refresh' style='border-radius:.6em;border: 1px solid #2A2A2A;background-image: -webkit-gradient(linear,left top,left bottom,color-stop(0,#666666),color-stop(1,#222222));background:#222222;margin:0px;height:60px;position:relative;text-align:center;line-height:60px;color:white;width:100%;'>" + this.refreshContent + "</div>");
                    }
                } else {
                    jqEl = jq(this.refreshElement);
                }
                var el = jqEl.get(0);
            ...
            scrollToItem: function (el, where) { //TODO: add functionality for x position
                ...
                if (where == 'bottom') {
                    itemPos = el.offset();
                    newTop = itemPos.top - this.jqEl.offset().bottom + itemPos.height;
                    newTop += 4; //add a small space
                } else {
                    itemTop = el.offset().top;
                    newTop = itemTop - document.body.scrollTop;
                    panelTop = this.jqEl.offset().top;
        ...
        jsScroller = function (el, opts) {
            ...
            this.container.jqScrollerId = el.jqScrollerId;
        ...
        nativeScroller.prototype.defaultProperties = function () {
            ...
            this.adjustScrollOverflowProxy_ = function () {
                ...
                that.jqEl.parent().css("overflow","hidden");
            };
        };
        ...
        jsScroller.prototype.startFormsMode = function () {
            ...
            var cssMatrix = this.getCSSMatrix(this.el);
            //toggle vars
            this.refreshSjqeKeep = this.refresh;
            this.refresh = false;
        ...
        jsScroller.prototype.stopFormsMode = function () {
            ...
            var cssMatrix = this.getCSSMatrix(this.el);
            //toggle vars
            this.refresh = this.refreshSjqeKeep;
            this.androidFormsMode = false;
            //set new css rules
